...
function walk(command) {
	...
	for (let i = 0; i < length; i++) {
		...
		const isNpmScript = subCmd.match(/^npm\s+run/) !== null;
...
function analyze(cmd) {
	const commandContent = scripts[cmd]; // 举例：npm run deploy 对应的脚本内容
	const finalCmds = walk(commandContent); // flatten 后得到的脚本
	return finalCmds;
}
